...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    mysqli_query($conn, "INSERT INTO ideas (startup_id, title, description, created_at) VALUES ('$startup_id', '$title', '$description', NOW())");
        ...
        foreach ($_FILES['media']['tmp_name'] as $key => $tmp_name) {
            if ($_FILES['media']['error'][$key] === 0) {
                ...
                if (move_uploaded_file($file_tmp, $target_path)) {
                    mysqli_query($conn, "INSERT INTO media (idea_id, media_type, file_path, created_at) 
                        VALUES ('$idea_id', '$file_type', '$new_name', NOW())");
                }
            }
        }
    }
