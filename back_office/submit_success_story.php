...
$synopsis = mysqli_real_escape_string($conn, $_POST['synopsis']); // ✅ New
$content = mysqli_real_escape_string($conn, $_POST['content']);

$insertStorySQL = "INSERT INTO success_stories (title, synopsis, content) 
                   VALUES ('$title', '$synopsis', '$content')"; // ✅ Add synopsis
mysqli_query($conn, $insertStorySQL);
$story_id = mysqli_insert_id($conn);

$uploadDir = 'uploads/success_stories/';

if (isset($_FILES['media_files']) && isset($_FILES['media_files']['name']) && is_array($_FILES['media_files']['name'])) {
